...
const NavigationMenu = React.forwardRef<React.ElementRef<typeof NavigationMenuPrimitive.Root>, React.ComponentPropsWithoutRef<typeof NavigationMenuPrimitive.Root>>(
  ({ className, children, ...props }, ref) => (
    <NavigationMenuPrimitive.Root ref={ref} className={cn("relative z-10 flex max-w-max flex-row items-center justify-center", className)} {...props}>
      {children}
...
const NavigationMenuList = React.forwardRef<React.ElementRef<typeof NavigationMenuPrimitive.List>, React.ComponentPropsWithoutRef<typeof NavigationMenuPrimitive.List>>(
  ({ className, ...props }, ref) => <NavigationMenuPrimitive.List ref={ref} className={cn("web:group flex flex-1 flex-row items-center justify-center gap-1 web:list-none", className)} {...props} />
);
...
const NavigationMenuTrigger = React.forwardRef<React.ElementRef<typeof NavigationMenuPrimitive.Trigger>, React.ComponentPropsWithoutRef<typeof NavigationMenuPrimitive.Trigger>>(
  ({ className, children, ...props }, ref) => {
    ...
    return (
      <NavigationMenuPrimitive.Trigger ref={ref} className={cn(navigationMenuTriggerStyle(), "web:group gap-1.5", value === itemValue && "bg-accent", className)} {...props}>
        <>{children}</>
        <Animated.View style={chevronStyle}>
          <ChevronDown size={12} className={cn("relative h-3 w-3 text-foreground web:transition web:duration-200")} aria-hidden={true} />
        </Animated.View>
    ...
    <NavigationMenuPrimitive.Portal hostName={portalHost}>
      <NavigationMenuPrimitive.Content
        ...
        className={cn(
          ...
          value === itemValue ? "web:animate-in web:fade-in web:slide-in-from-right-20" : "web:animate-out web:fade-out web:slide-out-to-left-20",
          className
        )}
        {...props}>
        <Animated.View entering={Platform.OS !== "web" ? FadeInLeft : undefined} exiting={Platform.OS !== "web" ? FadeOutLeft : undefined}>
          {children}
        </Animated.View>
      </NavigationMenuPrimitive.Content>
