...
import { Group, group } from "@/db/schema";
...
export default function Screen() {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      if (db) {
        await db.insert(group).values({ title: "Andrew" });

        const data = db.select().from(group).all();
        setGroups(data);
      }
    };
  ...
  return (
    <View className="flex-1 items-center justify-center gap-5 bg-secondary/30 p-6">
      <View className="flex flex-row gap-4">{groups && groups.map((group) => <Text key={group.id}>{group.title}</Text>)}</View>
      <Text>Hello</Text>
